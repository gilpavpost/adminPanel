<template>
  ...
  >
    ...
    <b-form-row>
      <b-col md="12">
        <b-form-group
          :label="$t('news.labels.categoryIds')"
...
export default {
  ...
  watch: {
    ...
    categorySelect(newValue) {
      if (newValue !== undefined) {
        this.form.categoryIds.push(newValue.id) ;
      }
    },
  },
  methods: {
    ...
    async load(id) {
      ...
      this.categorySelect = this.categories.find(
        (x) => String(x.id) === String(this.form.categoryIds)
      );
    },
    async filtersLoad() {
  ...
  async created() {
    ...
    if (this.internalId) {
      ...
      // this.categorySelect = this.categories[0]; Забивает массив сразу
    }
    this.loading = false;
  },
};
