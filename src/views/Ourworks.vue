          ...
          <b-row>
            <b-col order-lg="2" lg="auto">
              <b-button
                ...
                v-text="$t('ourworks.create')"
                block
              >
              </b-button>
            </b-col>
            <b-col order-md="1" col>
              <ourworks-search ref="searchForm" @search="search">
              </ourworks-search>
            </b-col>
          </b-row>
        </div>
...
import ourworksSearch from "@/components/ourworks/OurworksSearch";
import ourworksForm from "@/components/ourworks/OurworksForm";
...
export default {
  ...
  data() {
    return {
      ...
      tableOptions: {
        perPage: 10,
        headings: {
          id: this.$t("ourworks.id"),
          title: this.$t("ourworks.title"),
          prev_img: this.$t("ourworks.prev_img"),
          parentTitle: this.$t("ourworks.parentTitle"),
          status: this.$t("ourworks.status"),
          actions: "",
        },
        sortable: ["id", "title", "parentTitle", "status"],
        columnsClasses: {
          id: "id",
          actions: "actions",
          prev_img: "prev_img",
  ...
  computed: {
    tableColumns() {
      ...
      return ["id", "title", "prev_img", "parentTitle", "status", ...actions];
    },
  },
  methods: {
    imgUrl(rowItem) {
